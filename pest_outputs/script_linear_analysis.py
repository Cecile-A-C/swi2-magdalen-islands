...
hcond = [par_name for par_name in la.pst.par_names if par_name.startswith('hk') \
              and par_name in pst_bpa.adj_par_names] #                                    1) Hydraulic conductivities # in parlist_adj_nopp + parlist_adj_pp
...
par_dict = {'Hydraulic conductivities': hcond, 'Recharge': recharge, 'alpha_T': alpha}
...
# Add a 'name_model' column to the added_obs dataframe, to be able to use the merge function
added_obs_all_std_pct['name_model'] = added_obs_all_std_pct.index

# Merge data worth information (the added_obs dataframe) and geopandas geometry
hshallow, hdeep, hmuni, zwells, ztdem, zert, hobs, zobs = fpst.merge_df_with_gpd(pst_bpa, added_obs_all_std_pct, gis_path)
...
fpst.plot_figA1(pst_bpa, added_obs_all_std_pct, 'zmuni1', cmap, gis_path) # Fig. A.1

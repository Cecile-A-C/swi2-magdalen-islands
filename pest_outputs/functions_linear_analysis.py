...
def merge_df_with_gpd(df, hshallow_gp, hdeep_gp, hmuni_gp, zwells_gp, gis_path):
    
    # Read wells_obs.shp
    hwells_gdf = gpd.GeoDataFrame.from_features(records(gis_path + 'wells_obs' + '.shp', ['name_model']))
    zwells_gdf = copy.deepcopy(hwells_gdf)
    
    # Extract hshallow, hdeep and hmuni dataframes
    hwells_gdf.name_model = 'h' + hwells_gdf.name_model.astype(str)
    hshallow_gdf = hwells_gdf[hwells_gdf.name_model.apply(lambda x : x in hshallow_gp)]
    hdeep_gdf = hwells_gdf[hwells_gdf.name_model.apply(lambda x : x in hdeep_gp)]
    hmuni_gdf = hwells_gdf[hwells_gdf.name_model.apply(lambda x : x in hmuni_gp)]
    
    # Extract and create dataframe for zeta well observations
    zwells_gdf.name_model = 'z' + zwells_gdf.name_model.astype(str)
    zwells_gdf = zwells_gdf[zwells_gdf.name_model.apply(lambda x : x in zwells_gp)]
    
    # Read TDEM data
    ztdem_gdf = gpd.GeoDataFrame.from_features(records(gis_path + 'tdem' + '.shp', ['name_model']))
    ztdem_gdf.name_model = 'z' + ztdem_gdf.name_model.astype(str)
    
    # Read ERT data
    zert_gdf = gpd.GeoDataFrame.from_features(records(gis_path + 'ert_processed' + '.shp', ['name_model']))
        
    # For each observation group, merge dataframe with geopandas information
    
    hshallow = hshallow_gdf.merge(df, on='name_model')
    hdeep = hdeep_gdf.merge(df, on='name_model')
    hmuni = hmuni_gdf.merge(df, on='name_model')
    hobs = pd.concat([hshallow, hdeep, hmuni]) # Dataframe with all h observations
    
    zwells = zwells_gdf.merge(df, on='name_model')
    ztdem = ztdem_gdf.merge(df, on='name_model')
    zert = zert_gdf.merge(df, on='name_model')

...
# Create a dictionary containing all values at MNW2 wells for each iteration = {iteration_number: values for each well} 

i_start = 0
i_stop = active_mnw2_wells
starting_iteration = 0
mnw2_dict = {}

#for iteration in range(starting_iteration, nstp, output_nstp): # OPTION IF output_nstp NOT USED
for iteration in range(len(spd)):
    mnw2_dict[iteration] = mnw2_heads.iloc[i_start:i_stop, :] # Truncate the dataframe iteratively
    i_start += active_mnw2_wells
    i_stop += active_mnw2_wells
mnw2_last = mnw2_dict[ list(mnw2_dict)[-1] ] # Get last key of dictionary
...
##%%
## Post-process heads at model cells
#from functions_model_run import get_row_col_id, sim_to_df
#
#muni_df = get_row_col_id(muni_wells_row_col)
#
##for row col in ...
##for well in muni_df.name:
##    print(well)
#well='muni1'
#row = muni_df[muni_df.name == well]['row'].values[0]
#col = muni_df[muni_df.name == well]['col'].values[0]
#
#for i in range(len(spd)):
#    mnw2_key = i
#    print('hds well:')
#    print(row, col, hswiP[i,0,row,col])
#    print('hds well decalage:')
#    print(row+1, col+1, hswiP[i,0,row+1,col+1])
#    print('bynd (hwell):')
#    print(row+1, col+1, mnw2_dict[mnw2_key][mnw2_dict[mnw2_key].index == well]['hwell'][0])
#    print('bynd (hcell):')
#    print(row+1, col+1, mnw2_dict[mnw2_key][mnw2_dict[mnw2_key].index == well]['hcell'][0])
## DELETE FOR PAPER: PROBLEM: values in 'hcell' column in .bynd file are different from values from .hds file!!!
##    print(mnw2_dict[mnw2_key][mnw2_dict[mnw2_key].index == well]['hwell'][0])
##    hswiP[i,0,row,col] = mnw2_dict[0][mnw2_dict[0].index == well]['hwell'][0]
#
#hwells_df = sim_to_df(obs_h_row_col, hswiP, -1)
#for well in muni_wells_row_col.keys(): # Observations at municipal well = hwell (not hcell)

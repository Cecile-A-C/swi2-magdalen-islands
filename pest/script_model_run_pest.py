...
path_model_files = 'model_files/' # Path to model input files
path_model_outputs = 'model_outputs/' # Path to model output files
path_mnw2_data = path_model_files + 'muni_wells_mnw2_data.csv' # Path to the csv containing information for MNW2
path_krig_factors = 'pp_30max.fac' # Path to the file containing the kriging factors
path_hk_pp = 'hk1pp.dat' # Path to the file containing the hk values at pilot points
path_param_file = 'param.dat' # Path to the file containing other parameter values
...
param_df = pd.read_csv(path_param_file, index_col=0, delim_whitespace=True, header=None, names=['name','value'])
...
name_no_swi = 'idm_steady_no_swi_allwells' # Model name
...
name_swi = 'idm_transient_swi_allwells' # Model name
...
mnw2_heads = pd.read_table(name_swi + '.bynd', delimiter='\s+', usecols = [0,7,8], names = ['name', 'hwell', 'hcell'], header = 0, index_col=0) # Read the .bynd modflow output file as a dataframe
mnw2_heads.index = mnw2_heads.index.str.lower()
mnw2_last = mnw2_heads.tail(9)
...
hwells_df = fmod.sim_to_df(obs_h_row_col, hswiP, -1)

for well in muni_wells_row_col.keys(): # Observations at municipal wells: from hcell to hwell
    hwells_df.loc[hwells_df['name'] == well, 'value'] = mnw2_last['hwell'][well]

zwells_df = fmod.sim_to_df(obs_z_row_col, zetaP, -1)
ztdem_df = fmod.sim_to_df(tdem_row_col, zetaP, -1)
zert_df = fmod.sim_to_df(ert_row_col, zetaP, -1)
hdomest_df = fmod.sim_to_df(domestic_wells_row_col, hswiP, -1)
FW_storage_df = pd.DataFrame({'name': 'FW_storage', 'value': FW_storage})

#---- Remove TDEM outliers
...
#---- Remove ERT observations < 100 m to pumping wells

zert_df.drop(zert_df[ (zert_df['name'] == 'ert08') | 
        (zert_df['name'] == 'ert62') | (zert_df['name'] == 'ert71') | 
        (zert_df['name'] == 'ert72') | (zert_df['name'] == 'ert73') | 
        (zert_df['name'] == 'ert77') | (zert_df['name'] == 'ert78') | 
        (zert_df['name'] == 'ert79') ].index, inplace=True)
...
fmod.write_df_to_dat(path_model_outputs, 'hwells_sim', hwells_df, columns=['name','value'])
fmod.write_df_to_dat(path_model_outputs, 'zwells_sim', zwells_df, columns=['name','value'])
fmod.write_df_to_dat(path_model_outputs, 'ztdem_sim', ztdem_df, columns=['name','value'])
fmod.write_df_to_dat(path_model_outputs, 'zert_sim', zert_df, columns=['name','value'])
fmod.write_df_to_dat(path_model_outputs, 'fw_storage_sim', FW_storage_df, columns=['name','value'])
fmod.write_df_to_dat(path_model_outputs, 'hdomest_sim', hdomest_df, columns=['name','value'])
